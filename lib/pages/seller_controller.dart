...
class Sellercontroller extends GetxController{
  ...
  addproduct()async{
    ...
    productmodel myproduct=productmodel(pname: item_name.text.trim(), pdesc: description.text.trim(), pprice: base_price.text.trim(),sid: FirebaseAuth.instance.currentUser!.uid, pid: doc.id,pimage: item_image.text.trim(),pcategory: item_category,status: false,registeredusers: [],dateAdded: datechosen.toString().substring(0,10));
  ...
  print("Error shown in seller controller ${e}");
      ...
      productsshowinui1 =cameproducts.where((product) => product.pcategory == 'collectibles' && product.status == true ).toList();
      productsshowinui2 =cameproducts.where((product) => product.pcategory == 'antiques' && product.status == true).toList();
      productsshowinui3 =cameproducts.where((product) => product.pcategory == 'art'  && product.status == true).toList();
      productsshowinui4 =cameproducts.where((product) => product.pcategory == 'electronics'  && product.status == true).toList();
      productsshowinui5 =cameproducts.where((product) => product.pcategory == 'books'  && product.status == true).toList();
      productsshowinui6 =cameproducts.where((product) => product.pcategory == 'sport' && product.status == true).toList();
      productsshowinui7 =cameproducts.where((product) => product.pcategory == 'others' && product.status == true).toList();
...
final existingDateIndex = alldates.indexWhere((date) => date.date == selectedDate);
  if (existingDateIndex != -1) {
    // Check if all slots are filled
    if (alldates[existingDateIndex].slots.length == 5) {
      // Show snackbar indicating no slots available
      Get.snackbar('No Slots Available', 'All slots for this date are filled.');
    } else {
      // Slots are available
